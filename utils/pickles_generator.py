...
class Triplane_Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_config):
        ...
        if dataset_config.get("town", "all") == "all":
            towns = [
                folder for folder in os.listdir(self.data_path) if os.path.isdir(os.path.join(self.data_path, folder))
            ]
        else:
        ...
        for town in towns:
            if dataset_config.get("weather", "all") == "all":
                weathers = [
                    folder
                    for folder in os.listdir(os.path.join(self.data_path, town))
                    if os.path.isdir(os.path.join(self.data_path, town, folder))
                ]
            else:
            ...
            for weather in weathers:
                if dataset_config.get("vehicle", "all") == "all":
                    vehicles = [
                        folder
                        for folder in os.listdir(os.path.join(self.data_path, town, weather))
                        if os.path.isdir(os.path.join(self.data_path, town, weather, folder))
                    ]
                else:
                ...
                for vehicle in vehicles:
                    if dataset_config.get("spawn_point", "all") == "all":
                        spawn_points = [
                            folder
                            for folder in os.listdir(os.path.join(self.data_path, town, weather, vehicle))
                        ...
                        if dataset_config.get("step", "all") == "all":
                            steps = [
                                ...
                                for folder in os.listdir(
                                    os.path.join(self.data_path, town, weather, vehicle, spawn_point)
                                )
                                if "step_" in folder
                            ]
                        else:
...
def process_datapath(datapath):
    try:
        train_dataset = rays_dataset.RaysDataset(
            datapath,
            config,
            dataset_config=dataset_config.train_data_loader,
            mode="train",
