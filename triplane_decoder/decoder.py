...
class TriplaneDecoder(nn.Module):
    ...
    def forward(self, x_world, d=None, pif: PIF = None):
        ...
        if self.contraction:
            x_grid = contract_world(
                x_world.clone() * x_world.new_tensor(self.config.scene_contraction_factor)
            )  # samples, 3
        else:
            x_grid = (x_world - torch.tensor([self.offset_z, self.offset_h, self.offset_w], device=x_world.device)) / (
                torch.tensor(
                    [self.scale_z * self.N_z, self.scale_h * self.N_h, self.scale_w * self.N_w], device=x_world.device
                )
            )
        ...
        if pif is not None:
            pif_features = (
                pif(x_world.view(-1, 3), aggregate=True, num_features_to_keep=2)
                .permute(1, 2, 0)
                .reshape(x_world.size(0), -1)
            )  # samples, #2*img_features
    ...
    def update_planes(self, triplane):
        self.plane_zh = (
            triplane[2].view((self.N_z, self.N_h, -1)).unsqueeze(0).permute(0, 3, 1, 2).squeeze(0).permute(1, 2, 0)
        )
        self.plane_hw = (
            triplane[0].view((self.N_h, self.N_w, -1)).unsqueeze(0).permute(0, 3, 1, 2).squeeze(0).permute(1, 2, 0)
        )
        self.plane_zw = (
            triplane[1].view((self.N_z, self.N_w, -1)).unsqueeze(0).permute(0, 3, 1, 2).squeeze(0).permute(1, 2, 0)
        )
