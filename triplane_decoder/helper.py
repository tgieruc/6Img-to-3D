    ...
    for img_index in pbar:
        ...
        for i in range(int(np.ceil(H / chunk_size))):
            ...
            regenerated_px_values = render_rays(
                model, ray_origins_, ray_directions_, hn=hn, hf=hf, nb_bins=nb_bins, box_rendering=box_rendering
            )
    ...
    for epoch in range(nb_epochs):
        ...
        for n_batch, batch in enumerate(pbar):
            ...
            if mask.sum() > 0:
                ...
                regenerated_px_values = render_rays(
                    model, ray_origins, ray_directions, hn=hn, hf=hf, nb_bins=nb_bins, box_rendering=box_rendering
                )
                ...
                if visualize_every > 0:
                    if n_batch % visualize_every == 0:
                        triplane = [[model.xy_plane.detach(), model.xz_plane.detach(), model.yz_plane.detach()]]
