...
def compute_accumulated_transmittance(alphas):
    ...
    return torch.cat(
        (
            torch.ones((accumulated_transmittance.size(0), 1, 1), device=alphas.device),
...
def render_rays(
    ...
    **kwargs,
    ...
    uniform_sampler = ray_sampler.UniformSampler(
        num_samples=config.decoder["nb_bins"], train_stratified=config.decoder["train_stratified"]
    )
    pdf_sampler = ray_sampler.PDFSampler(
        num_samples=config.decoder["nb_bins"],
        train_stratified=config.decoder["train_stratified"],
    ...
    if only_coarse:
        colors = volume_rendering(
            samples_coarse.deltas, colors_coarse, densities_coarse, config.decoder.white_background
        )
    ...
    colors = volume_rendering(samples_fine.deltas, colors_fine, densities_fine, config.decoder.white_background)
...
def volume_rendering(
    deltas: torch.Tensor, colors: torch.Tensor, sigma: torch.Tensor, white_background: bool
) -> torch.Tensor:
