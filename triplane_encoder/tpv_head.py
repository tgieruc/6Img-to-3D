...
class TPVFormerHead(BaseModule):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        pose_embeds = (
            torch.from_numpy(np.stack([m["pose_intrinsics"] for m in img_metas])).to(mlvl_feats[0].device).to(dtype)
        )  # (B, N, 20)
...
class CustomPositionalEncoding(BaseModule):
    def __init__(self, num_feats, h, w, z, init_cfg=dict(type="Uniform", layer="Embedding")):
    ...
    def forward(self, bs, device, ignore_axis="z"):
        ...
        pos = torch.cat((h_embed, w_embed, z_embed), dim=-1).flatten(0, 1).unsqueeze(0).repeat(bs, 1, 1)
        return pos
