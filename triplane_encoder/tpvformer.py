...
class TPVFormer(BaseModule):
    ...
    def extract_img_feat(self, img):
        ...
        if img is not None:
            ...
            if isinstance(img_feats, dict):
                img_feats = list(
                    img_feats.values()
                )  # [torch.Size([6, 512, 116, 200]), torch.Size([6, 1024, 58, 100]), torch.Size([6, 2048, 29, 50])]
        else:
            return None
        if hasattr(self, "img_neck"):
            img_feats = self.img_neck(
                img_feats
            )  # [torch.Size([6, 128, 116, 200]), torch.Size([6, 128, 58, 100]), torch.Size([6, 128, 29, 50]), torch.Size([6, 128, 15, 25])]
    ...
    def forward(
        ...
        triplane = self.tpv_head(
            img_feats, img_metas
        )  # [torch.Size([1, 40000, 128]), torch.Size([1, 3200, 128]), torch.Size([1, 3200, 128])]
