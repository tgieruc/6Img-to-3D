...
class TPVCrossViewHybridAttention(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.output_proj = nn.ModuleList([nn.Linear(embed_dims, embed_dims) for _ in range(3)])
        self.sampling_offsets = nn.ModuleList([nn.Linear(embed_dims, num_heads * 3 * num_points * 2) for _ in range(3)])
        self.attention_weights = nn.ModuleList(
            [nn.Linear(embed_dims, num_heads * 3 * (num_points + 1)) for _ in range(3)]
        )
        self.value_proj = nn.ModuleList([nn.Linear(embed_dims, embed_dims) for _ in range(3)])
    ...
    def init_weight(self):
        ...
        # self plane
        theta_self = torch.arange(self.num_heads, dtype=torch.float32) * (2.0 * math.pi / self.num_heads)
        grid_self = torch.stack([theta_self.cos(), theta_self.sin()], -1)  # H, 2
        grid_self = grid_self.view(self.num_heads, 1, 2).repeat(1, self.num_points, 1)
        ...
        if self.init_mode == 0:
            ...
            theta = torch.arange(num_theta, dtype=torch.float32) * (math.pi / num_theta) + (
                math.pi / num_theta / 2
            )  # 3
            ...
            xyz = [[0, 0, 1], [0, 0, -1], [0, 1, 0], [0, -1, 0], [1, 0, 0], [-1, 0, 0]]
        ...
        for i in range(3):
            ...
            xavier_init(self.value_proj[i], distribution="uniform", bias=0.0)
            xavier_init(self.output_proj[i], distribution="uniform", bias=0.0)
