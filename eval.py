        ...
        for scene_index, (imgs, img_metas, val_dataset) in tqdm(
            ...
            for img_index in index_set:
                ...
                for i in range(int(np.ceil(H / cfg.decoder.testing_batch_size))):
                    ...
                    regenerated_px_values, _, depth = render_rays(
                        triplane_decoder,
                        ray_origins_,
                        ray_directions_,
                        cfg,
                        pif=pif,
                        training=False,
                        only_coarse=args.single_sampling,
                    )
                ...
                ssim_metric = ssim(
                    img.view(1, H, W, 3).permute(0, 3, 1, 2),
                    ground_truth_image.view(1, H, W, 3).permute(0, 3, 1, 2),
                    data_range=1,
                    size_average=True,
                ).item()
                ...
                if args.gif:
                    ...
                    imageio.imwrite(
                        os.path.join(logdir, "imgs", f"{scene_index}_{img_index}.png"),
                    ...
                    if args.img_gt:
                        imageio.imwrite(
                            os.path.join(logdir, "imgs_gt", f"{scene_index}_{img_index}.png"),
                            (255 * ground_truth_image).cpu().numpy().astype(np.uint8),
                        )
                        if args.depth and depth_gt is not None:
                            imageio.imwrite(
                                os.path.join(logdir, "imgs", f"{scene_index}_{img_index}gt_depth.png"), depth_gt
                            )
                if args.gif_gt:
                    gif_gt.append(
                        Image.fromarray((255 * ground_truth_image).view(H, W, 3).cpu().numpy().astype(np.uint8))
                    )
                    if args.depth and depth_gt is not None:
            ...
            if args.gif:
                gif[0].save(
                    os.path.join(logdir, "imgs", f"{scene_index}.gif"),
                    save_all=True,
                    append_images=gif[1:],
                    duration=50,
                    loop=0,
                ...
                if args.depth:
                    depth_gif[0].save(
                        os.path.join(logdir, "imgs", f"{scene_index}_depth.gif"),
                        save_all=True,
                        append_images=depth_gif[1:],
                        duration=50,
                        loop=0,
                    )
            if args.gif_gt:
                gif_gt[0].save(
                    os.path.join(logdir, "imgs", f"{scene_index}gd.gif"),
                    save_all=True,
                    append_images=gif_gt[1:],
                    duration=50,
                    loop=0,
                )
                if args.depth:
                    depth_gif_gt[0].save(
                        os.path.join(logdir, "imgs", f"{scene_index}gd_depth.gif"),
                        save_all=True,
                        append_images=depth_gif_gt[1:],
                        duration=50,
                        loop=0,
                    )
